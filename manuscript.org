...
* Contents                                                   :TOC@4:noexport:
 ...
 - [[#inprogress-le-détournement-à-travers-lhistoire-des-langages-de-programmation][INPROGRESS Le détournement à travers l'histoire des langages de programmation]]
   ...
   - [[#read-séparer-les-préoccupations-la-programmation-par-aspects][READ Séparer les préoccupations: la programmation par aspects]]
     - [[#la-distinction-entre-composants-et-aspects][La distinction entre composants et aspects]]
   ...
   - [[#travaux-connexes-concernant-linstrumentation][Travaux connexes concernant l'instrumentation]]
     - [[#virtual-values-and-proxies][Virtual values and proxies]]
 ...
 - [[#construire-un-interpréteur-par-modules][Construire un interpréteur par modules]]
   ...
   - [[#unread-discussion][UNREAD Discussion]]
     ...
     - [[#manipuler-la-portée-suffit-pour-détourner-le-programme][Manipuler la portée suffit pour détourner le programme]]
...
Implémenter une analyse c'est essentiellement implémenter à un interpréteur.
...
construire un nouveau.  Mais, comme on le montrera ([[#problem]]), cette solution
n'est pas non plus idéale pour implémenter plusieurs analyses.  Dans un contexte
de recherche, il faut pouvoir itérer rapidement pour couvrir l'espace des
solutions.  Prototyper des analyses pour des programmes JavaScript devrait donc
être rapide.  Mais dans le même temps, il ne faut pas non plus sacrifier la
...
([[#idea]]).  Détourner un interpréteur repose sur l'utilisation de mécanismes de
langage qui nous permettent de modifier le comportement d'un programme sans
modifier directement son code source: des mécanismes tels que le liaison
dynamique, les variables à portée dynamique ou la métaprogrammation ([[#chrono]]).

Pour illustrer le détournement, on utilisera de simples interpréteurs que l'on
modifiera en utilisant différentes techniques ([[#variations]] [[#foal]]).  Puis, pour
démontrer l'utilité du détournement sur un interpréteur réaliste, on applique
ces techniques à Narcissus ([[#dls]]), un interpréteur de JavaScript.  On montre
comment, grâce au détournement, on peut facilement ajouter des analyses
dynamiques de programmes sans impacter le code source de l'interpréteur.
...
like to write and to explain what they are doing. — [[cite:Knu-84][Knu-84]]
#+END_epig
...
#+ATTR_HTML: :style margin-top:-3.5rem
#+BEGIN_aside
...
Le protocole métaobjet de CLOS (CommonLisp Object System) [[cite:KRB-91][KRB-91]] va encore plus
loin, en exposant au programmeur le protocole qui implémente les mécanismes de
programmation par objets dans ce langage.  Pour réconcilier les différentes
implémentations de programmation par objets qui existait en CommonLisp (Flavors,
LOOPS), CLOS laisse au programmeur la possibilité de redéfinir l'héritage entre
classes, la liaison dynamique, ou la visibilité des membres.  En Python ou en
Java, il y une structure de classe imposée, et un mécanisme de liaison par
défaut qu'il n'est pas toujours possible de redéfinir.  En CLOS, ce qui est
fourni est avant tout le protocole métaobjet; puis une implémentation par défaut
de ce protocole, mais qui est en tout point redéfinissable par le programmeur.
Autrement dit, le programmeur ne programme plus seulement des applications, mais
il programme en partie le langage.  Il programme le /programme/ lui-même, d'où
le terme de métaprogrammation.
...
#+BEGIN_side-figure
#+CAPTION: Dans une décomposition fonctionnelle classique, les opérations
#+CAPTION: implémentent du code pour chaque variante du langage
#+CAPTION: (sens des flêches).  Ajouter une opération (à gauche) n'affecte
#+CAPTION: pas les unités existantes, mais ajouter un terme
...
*** Problème inverse en programmation par objets
:PROPERTIES:
:UNNUMBERED: t
:END:
...
FP: open function; Swierstra things

OOP: visitor (Krishnamurthi), Zenger Odersky, Oliveira
...
#+BEGIN_side-figure
#+CAPTION: Représentation des objets en mémoire après la création de ~e1~.
[[file:img/foal-1.svg]]
#+END_side-figure
...
#+BEGIN_side-figure
#+CAPTION: Après les ajouts ({{{color(c1)}}}) de ~plus~ et ~e2~.
[[file:img/foal-2.svg]]
#+END_side-figure
...
#+BEGIN_side-figure
#+CAPTION: Ajouter ~s~ n'affecte ni ~e1~ ni ~e2~: c'est un nouvel objet qui
#+CAPTION: réutilise ~num~ et ~plus~ sans les modifier.
[[file:img/foal-4.svg]]
#+END_side-figure
