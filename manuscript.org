...
* Contents                                                   :TOC@2:noexport:
 ...
 - [[#le-détournement-à-travers-lhistoire-des-langages-de-programmation][Le détournement à travers l'histoire des langages de programmation]]
   ...
   - [[#unread-autres-mécanismes-de-détournement][UNREAD Autres mécanismes de détournement]]
   - [[#unread-récapitulatif][UNREAD Récapitulatif]]
...
** UNREAD Autres mécanismes de détournement
*** La superposition
...
Hyper/J est un système pour séparation pluridimensionnelle des préoccupations
pour Java [[cite:TOH+99][TOH+99]] [[cite:TO-00][TO-00]].  C'est un paradigme qui permet de séparer les
...
représentent les termes apparaissent toutes les hypercoupes.
#+END_side-figure
...
hypercoupe pour synthétiser le programme.  Composer le code source se fait à
...
L'application principale est de pouvoir modifier des drivers lorsque l'interface
...
Ce principe s'applique pas seulement au code.  Une pratique courante est
...
paradigme.  En contrepartie, elle n'offre aucune garantie.  Si l'on malpelle
~TODO~ en ~TOOD~, la note ne sera peut être oubliée à jamais.
...
peut être redéfinit par le programmeur.  Par exemple, lorsqu'une valeur
...
de valeurs.  Ils donnent l'exemple d'une fonction ~delay~ qui prend comme
argument une fonction qui représente le calcul à mémoriser.  La valeur retournée
par ~delay~ est une valeur virtuelle, qui se comporte comme une valeur
...
mémorisé.  Le calcul est donc effectué au plus seule fois, de façon transparente
pour le programme.
...
#+BEGIN_SRC js
var v = delay(() => large_computation_returns_an_int())
...
néanmoins que l'interpréteur supporte les valeurs virtuelles dans un premier
temps.  Mais une fois cette modification effectuée, les différentes stratégies
...
réalisables à travers les valeurs virtuelles.  Comme une valeur virtuelle est
...
le comportement du programme.  Par exemple, la mémorisation est utile pour des
valeurs ponctuelles, mais les valeurs virtuelles ne permettent pas de
transformer tout le langage en langage à évaluation paresseuse.
...
n'est cependant pas aussi riche des raisons de compatibilité.  Néanmoins, Keil
et Thiemann ont utilisé ces mandataires avec succès pour améliorer leur moniteur
d'effets JSConTest qui était précédemment basé sur de la transformation de code
[[cite:KT-13][KT-13]].
...
prend l'exemple d'associer temporairement une séquence de touche à une action,
...
façon générale, la portée dynamique permet à l'appelant de surcharger les
...
variables à portée dynamique, et d'autres à portée lexicale.  Moreau note que la
portée dynamique apporte de l'expressivité à un langage purement fonctionnel
[[cite:Mor-98][Mor-98]], mais que la plupart des langages courants n'offrent que la portée
lexicale.  Il donne l'exemple de Perl, TeX, CommonLisp et Bash comme langages
majeurs à portée dynamique.  Comme Queinnec le montre, on peut émuler la portée
dynamique dans une fonction en lui passant un argument supplémentaire d'où
retirer ses variables [[cite:Que-03][Que-03]].  Néanmoins, le langage Haskell implémente tout de
même la portée dynamique à travers l'extension des paramètres implicites [[cite:LLM+00][LLM+00]].

** UNREAD Récapitulatif
Dans tous les travaux que l'on a étudié dans ce chapitre il y a un thème qui
...
extensible, que faire?  On peut le réecrire, pour adapoter l'un des paradigmes
...
~COMEFROM~ ensembles de cette façon:
...
Quand on utilise des étiquettes, on peut considérer que l'on vient modifier le
code ciblé.  Techniquement il s'agit bien d'une modification, mais on peut aussi
le voir comme une simple annotation.  L'ajout d'une étiquette ne change pas la
sémantique du code, tant que cette étiquette n'est pas utilisée.
...
- org mode example
- still need explicit labels at places where we want to insert code later
- diverting happens statically, when code is tangled

OOP:
- As Kay noted, message passing is already flexible because of inheritance.  So
  if we define an interpreter as having an ~eval~ method, we can inherit and
  override this behavior later.
- Use inheritance to override code: works but of limited use?
- Mostly the solutions in \sect Expression Problem cover this well.
- But here we are diverting, not constructing.  How do they apply?
- Downside is that the interpreter might not have an ~eval~ method at all.  Very
  dependent on the interpreter structure.
...
programme, il faut introduire de l'indirection.  Et de façon générale, cette
morale vaut aussi pour étendre un programme.  Si les références et les appels de
méthodes sont indirects, ils seront plus faciles à changer.
...
difficile à comprendre, à prédire.  On ne peut plus garantir le comportement
d'une fonction en isolation, il faut prendre en compte toutes les extensions que
ces indirections rendent possibles.  Même dans un simple patron observateur, on
ne peut pas prédire ce que que le programme fera sans connaître tous les
observateurs enregistrés auprès d'un sujet.  Et potentiellement l'ensemble de
ces observateurs ne pourra être déterminé qu'à l'exécution du programme.  Le
détournement a le même inconvénient.  Il y a donc un compromis: plus le
programme est facile à étendre ou détourner, et moins il sera facile d'en
prédire son comportement en le lisant ou l'analysant.  Le détournement n'est pas
une balle en argent; pas plus que la refactorisation.  L'enseignement ici est
qu'il faut être conscient des conséquences de ces compromis lors de la
conception et tout au long de l'évolution du programme.
