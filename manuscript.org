...
Seplution wants to minimize impact of change, abolish symmetry to avoid dominant
decomposition.  Applies to all artefacts pertaining to preoccupation, not only
code.  Goes beyond AOP, and into FOP and MDD territory, that is not our path.

HyperJ realizes seplution for source code in Java.  Write Java in hyperslices,
compile to recompose program.  The merging mechanism is basically
superimposition.
