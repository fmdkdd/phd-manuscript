...
* Contents                                                   :TOC@4:noexport:
 ...
 - [[#une-chronologie-du-détournement-dans-les-langages-de-programmation][Une chronologie du détournement dans les langages de programmation]]
...
* Une chronologie du détournement dans les langages de programmation
:PROPERTIES:
:CUSTOM_ID: chrono
:END:
...
La programmation prône donc d'utiliser des langages qui maintiennent le système
implicite de coordonnés de Dijkstra.  Des langages comme ALGOL et Pascal, dans
lesquels les procédures n'ont qu'un seul point d'entrée et un seul point de
sortie, et où les ~goto~ et autres sauts arbitraires sont proscrits.
...
code/).  Et s'il est vrai que le ~goto~ peut être remplacé dans la plupart des
cas par des constructions de boucles appropriées (~do while~, ~loop then~), son
cousin moins populaire, ~comefrom~ offre des possibilités sans égales.

[segue comefrom as first diverting example]
...
with (state({num, plus})) {
  with (double({num})) {
    with (show({num, plus})) {
      ...
      [
      getCount(),
      n = plus.new(num.new(1), num.new(2)),
      n.eval(),
      getCount(),
      n.show(),
      ]
}}} //: [0,Object {l:Object {n:1},r:Object {n:2}},6,3,"1+2"]
...
p2(show(state(double({num, plus})))) //: [0,6,3,"1+2"]
#+END_SRC
