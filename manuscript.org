...
* Contents                                                   :TOC@4:noexport:
 ...
 - [[#construire-un-interpréteur-par-modules-45][Construire un interpréteur par modules [4/5]]]
   ...
   - [[#unwritten-discussion][UNWRITTEN Discussion]]
...
* Construire un interpréteur par modules [4/5]
...
** UNWRITTEN Discussion
- goal achieved: extend the language and the interpreter without changing a line
  of the preceding code.

- main mechanisms at hand: delegation to extend, and scope manipulation with
  ~with~.  The main idea is: the same code block has different results depending
  on what's in scope.  With ~with~, we put what we want into the scope.

  That's diverting.

- example is a bit contrived, as we have to create the AST expression /inside/
  ~with~.  We cannot create it beforehand, then use ~with~ to change its
  evaluation.  To see that, understand that ~plus.new~ is the same as
  ~double.plus.new~ when ~double~ is active.  We could be explicit when calling
  these terms.

  A better solution would have been to have ~with(double)~ modify the ~eval~
  directly.  That's a bit what COP in JS [ref] gives you.

- Nevertheless, this idea of scope manipulation can be applied to Narcissus
  itself, segue.
