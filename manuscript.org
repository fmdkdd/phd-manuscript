...
* Contents                                                   :TOC@4:noexport:
 ...
 - [[#le-problème-étendre-des-interpréteurs][Le problème: étendre des interpréteurs]]
   ...
   - [[#bare-le-but-factoriser-les-changements-en-minimisant-les-changements-du-code-source][BARE Le but: factoriser les changements en minimisant les changements du code source]]
...
** BARE Le but: factoriser les changements en minimisant les changements du code source
D'un point de vue abstrait, on peut voir l'interpréteur comme une fonction
mathématique qui associe un processus à un code source:
...
Mais généralement, la modification est relativement mineure.  On ne cherche pas
à totalement changer le sens de l'interprétation, sinon on pourrait tout aussi
bien écrire un nouvel interpréteur.  Si l'on choisit de modifier un interpréteur
existant, c'est parce qu'en grande partie sa spécification est réutilisable.  La
spécification modifiée est donc dérivée de la spécification d'origine.  On peut
le noter ainsi:

: spec_1 -modification-> spec_2

La modification décrit uniquement les changements, le /delta/, entre les deux
spécifications.  C'est ce delta qui nous intéresse quand on veut implémenter
cette modification.

Il y a plusieurs façons d'implémenter cette modification.  Rappelons nous que le
processus de développement est le suivant:

: Spécification -programmeur-> Code source -compilateur-> Programme -machine-> Résultats

#+BEGIN_aside
Le véritable objet du programmeur est l'exécution du programme.  Mais il ne
manipule pas directement l'exécution de la machine, il décrit les instructions à
l'avance.
#+END_aside

Le programmeur élabore le code source en fonction de la spécification.  Le but
est de concevoir un programme qui aura les résultats attendus.  Pour s'en
assurer, le programmeur observe le résultat de l'exécution du programme.  Si ces
résultats sont en accord avec la spécification, le programme est fini.  Sinon,
il faut modifier le code source pour modifier le processus engendré, jusqu'à ce
que les résultats concordent.

Dans la section précédente, ce delta est perdu dans les modifications du code
source de l'interpréteur.  Il n'y a pas de marques apparentes dans le code
source qui indiquent que cette partie est modifiée ou non.
...
Le but de l'extension de Narcissus était d'ajouter l'évaluation à facettes.  Les
auteurs ont choisi de modifier directement le code source de l'interpréteur.
C'est une solution simple et directe, mais qui a les inconvénients listés
précédemment.  Comment
