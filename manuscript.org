...
D'après eux, le mécanisme essentiel de la programmation par aspect c'est de
permettre de faire des déclarations de la forme:
...
#+BEGIN_aside
Encore dit autrement, le mécanisme n'est pas différent d'un ~COMEFROM~ [[cite:Cla-73][Cla-73]],
le dual du ~GOTO~.
#+END_aside

Autrement dit, un système d'aspects /quantifie/, au sens logique, les programmes
et les conditions, afin d'exécuter du code.  Les systèmes d'aspects varient
suivant ce qu'ils permettent de quantifier.  Les systèmes boîtes noires
quantifient l'interface publique des composants: les fonctions et les méthodes
d'objets uniquement.  Les systèmes transparents quantifient l'arbre de syntaxe:
affectation de variables, structures de contrôle, distinction entre envoi et
réception de message.  AspectJ et le langage d'aspect pour les filtres d'images
sont tous deux des systèmes transparents dans cette classification.
...
particulier, les systèmes à événements permettent la quantification; si les
composants ne communiquent qu'à travers des événements, on peut écrire des
aspects qui quantifient ces événements, sans impacter le reste du système (les
aspects sont oubliables).  Un système à événements peut donc être utilisé pour
