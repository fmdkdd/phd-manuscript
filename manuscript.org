...
offre de nouvelles façons de concevoir des programmes extensibles.
...
instances, sans connaître leurs noms statiquement.  Ligne [[(synth)]] on crée
une instance de ~A~, ~anA~, et on invoque la méthode ~m2~ récupérée elle aussi
par réflexion.  Invoquer une méthode dynamiquement à partir d'une chaîne qui
contient nom rend le programme plus flexible: ça permet d'implémenter simplement
l'appel de procédure distante (RPC).  On peut aussi récupérer ou changer la valeur un
attribut dynamiquement, ce qui est utile pour écrire des objets qui font la
passerelle avec des bases de données relationnelles (ORM).
...
#+BEGIN_aside
Dans la formulation du problème par Wadler, il y a deux contraintes: l'extension
ne doit pas nécessiter de recompilation, et doit préserver le typage statique.
Pour nous, ces deux contraintes ne sont pas nécessaires pour rendre le problème
intéressant.  Elle le rendent certes plus difficile, mais l'extension
d'interpréteur est avant tout un problème d'extensibilité, et s'applique aussi
bien à des langages interprétés (non compilés), et typés dynamiquement.
#+END_aside
...
- Asymmetry between FP and OOP that exhibit a different choice of dominant
  decomposition.

Why is it difficult?  It's only three lines here.

- It's about modularity.  In a larger interpreter, each operation would be its
  own module.  Adding an operation would impact only one module.  Adding a term
  impacts the term module, and all the operation modules.

How is it relevant to extensibility?

- It's about extending an interpreter, a program.

What's the takeaway?

- Asymmetry is interesting
- Spawned many solutions that we'll use as a start to extending Narcissus
- Mostly trivial in dynamic languages
...
Mais le problème de l'expression ne s'intéresse qu'à étendre un langage
