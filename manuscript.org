...
*** TRANSLATE Hyper/J
Hyper/J est un autre système qui permet la séparation des préoccupations [[cite:TOH+99][TOH+99]].
Dans la programmation par aspects, l'axe des composants est l'axe principal et
les aspects offrent un axe de recomposition orthogonal, mais secondaire.  De
même, Hyper/J offre plusieurs axes de composition du programme.  Mais
contrairement aux aspects, ici aucun axe ne prime sur l'autre.

Les auteurs remarquent que les formalismes logiciels existants permettent des
mécanismes pour décomposer et recomposer les logiciels en unités gérables par le
programmeur: modules, classes, méthodes, procédures.  Ces unités permettent de
séparer les préoccupations du domaine d'application.  Mais la décomposition est
en général insuffisante, car il y a toujours une dimension qui prédomine sur les
autres; c'est ce qu'ils nomment "la tyrannie de la décomposition dominante".

Dans un système, il y a toujours des plusieurs préoccupations qu'il faut
équilibrer, et la décomposition dominante en favorise certaines au détriment
d'autres.  C'est une preuve que le système a en réalité plusieurs dimensions, et
chaque décomposition n'est qu'une projection, une vue, du système dans un espace
de dimension inférieure.  Aucune vue n'est satisfaisante, car il y a toujours
une dimension qui n'est pas prioritaire.

Ils proposent donc des /hypercoupes/: des façons d'organiser le système selon
toutes les dimensions qui ont rapport à une préoccupation.  Une hypercoupe
contient toutes les unités de changements de cette préoccupation.  Et les unités
peuvent apparaître dans plusieurs hypercoupes.
