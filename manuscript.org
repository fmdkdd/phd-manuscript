...
Pour un déterministe, tout remerciement est à la fois superflu et insuffisant.
Si tous les évènements qui ont coloré mon parcours durant ces quatre dernières
années étaient inéluctables, si chaque personne que j'ai croisé n'a fait que
jouer son rôle incombé par les lois de la physique, si aucun — moi compris – n'a
eu de choix, à quoi bon se congratuler?  Ou alors, il faudrait remercier
l'univers entier, chaque molécule, chaque atome pour leur agencement, chaque
minuscule cause pour ses conséquences qui, au passage, m'ont touché
personnellement.  Pourquoi remercier les causes les plus proches, mais pas
toutes celles qui les ont précédées?

Et pourtant, le moment venu, c'est non sans plaisir que je peux dire:

Merci à Wilfried, Michel, Françoise, Pierre et Isabelle de m'avoir fait tomber
dans l'ordinateur quand j'étais tout petit.  Merci à Julien, Jérôme et Karim
pour m'avoir suggéré de poursuivre des études d'informatique.  Merci à Santiago,
Étienne, Jean-Pierre, Véronique, Damien et Cyrille pour m'avoir donné envie
d'enseigner et pour m'avoir permis de le faire.  Merci à Reginald pour m'avoir
fait découvrir la programmation par aspects.  Merci à Bret pour m'avoir inspiré
d'explorer la dissonance entre l'écrit et le manipulé.  Merci à Jacques et Mario
pour m'avoir choisi pour cette thèse et pour tout ce qui s'en est suivi; on n'a
pas toujours pu s'entendre, mais on a su faire avec.  Un merci particulier à
Jacques pour tes relectures consciencieuses ces derniers mois.  Merci aussi à
Stéphane, Christophe et Guillaume, pour avoir eu la patience de lire ce
document.  Merci à Éric de m'avoir accueilli à Santiago et pour les bonnes
adresses culinaires.  Merci à Alexandre pour ta générosité.  Merci à Johan pour
la lecture durant mon séjour.  Merci à Ismael pour m'avoir aidé à déchiffrer des
monades et les erreurs de GHC.  Merci à Alan et Martin pour nos échanges sur JS,
et pour les coups de main en Coq à l'EJCP.  Thanks to Paolo, Arlen, Jim, Ben for
the pleasant conversations in Fukuoka, Montpellier or San Francisco.  Merci à
Nicolas et Pierre-Marie pour les jeux de mots graveleux, à Simon, Kevin, Rafael,
Théo, Benedikt, Paige, Adrien, John, Anthony, Ronan, Brice, Adrien (le
lointain), Simon (l'autre) et Alex pour toutes les discussions enrichissantes, à
table, au café, ou ailleurs.  Merci aux antagonistes pour m'avoir montré le
chemin à ne pas suivre.  Merci aux participants de Papredi pour avoir soutenu le
partage de connaissances au sein d'une équipe éparpillée.  Merci à Hervé pour
avoir toujours cherché à m'intégrer.  Merci à Rémi d'avoir complété mes phrases
par des absurdités improvisées.  Merci aux complices d'Arcadi pour tous ces bons
moments à la dérobée.  Merci au crou B218, Walid et Ronan, de m'avoir supporté,
moi et mon clavier, pendant toutes ces années.  Merci particulier à Ronan pour
ta diligence exemplaire.  Merci à Véro, Muriel, consorts et marmaille qui
rendent Nantes bien plus familière.  Merci à Merwan d'avoir un jour demandé à
être mon binôme.  Et merci à Marie, pour être toujours à mes côtés.
...
Parnas nous dit qu'il suffit de dessiner l'organigramme pour obtenir les 5
...
une boucle infinie.  Mais il y a une différence notable: ~GOTO~ influe sur le
...
Ici on crée, en utilisant la fonction ~type~, une classe ~C~ qui contient une
...
intéressés à appliquer la réflexion à d'autres systèmes, ce qu'ils appellent
...
complexité d'un algorithme.  L'opération ~pp~ comporte autant de lignes qu'il y
a de variantes de ~Term~, plus une pour sa signature.  S'il y a /v/ variantes,
...
On ne modifie pas ~Visitor~; on ajoute seulement une interface pour ~Mult~.
...
manière équivalente, on pourrait retourner une instance de l'interface ~PP~ qui
...
#+ATTR_LATEX: :options [1em]
#+BEGIN_side-figure
[[file:img/foal-legend.svg]]

Légende des diagrammes qui accompagnent les exemples en JavaScript.
#+END_side-figure

Le diagramme à côté du code illustre les objets manipulés par l'interpréteur
JavaScript.  Les objets y sont représentés par des boîtes qui contiennent des
paires, chaque paire associant une /clé/ à une /valeur/.  Lorsque la valeur est
une référence, elle est représentée par une puce d'où part un trait vers l'objet
référencé ([[file:img/legend1.svg]]).  Par exemple, ici ~num~ est une référence vers
un objet qui contient les clés ~new~ et ~eval~.  Habituellement, on désigne un
objet par la clé qui le réfère: on parlera ici de l'objet ~num~, de l'objet
~n1~, etc.  Une puce qui part du coin d'un objet représente le lien de
délégation vers l'objet parent (le prototype).  Par exemple, ~n1~ a ~num~ pour
parent (l'objet référencé par ~n1~ a pour parent l'objet référencé par ~num~).
Les références en pointillés sont omises par concision: ici ~new~ est une
référence vers une fonction qui a peu d'importance pour la suite.

On a plusieurs façons de modifier l'évaluation de ~n1~ après sa définition.  On
peut surcharger la fonction sur l'objet lui-même:
...
~interp_double~ redéfinit la règle ~num~ et délègue ~eval~ et ~plus~ à
~interp~.  Le lien de délégation est indiqué par
[[file:img/variations-legend.svg]].
#+END_side-figure
...
portée dynamique, les variables libres d'une fonction peuvent être définies
...
associative ~eval-rules~.  Cette liste est stockée dans une variable que l'on va
pouvoir modifier par portée dynamique:
...
Représentation des objets en mémoire après la création de ~e1~.  Une légende et
une explication de ces diagrammes apparaissent {{{ref(diag-legend, là)}}}.
#+END_side-figure
...
contiennent juste l'opération ~show~ définie comme précédemment.  Enfin, la
...
des mécanismes qui désambiguïsent les noms.
...
#+BEGIN_aside
Pour un rappel de la légende des diagrammes, voir {{{ref(diag-legend, là)}}}.
#+END_aside

On peut expliquer le motif module en regardant le diagramme qui accompagne le
code.  Dans le diagramme, les environnements, comme les objets, sont représentés
par des boîtes.  Une puce dans le coin d'un environnement indique le lien vers
l'environnement parent: par exemple l'environnement qui contient ~a~, ~f~ et ~g~
a pour parent l'environnement qui contient ~m~.  Lorsqu'un nom est recherché
dans un environnement et que ce nom n'est pas présent dans les propriétés de cet
environnement, la recherche continue dans l'environnement parent, et
récursivement jusqu'à ce que soit la propriété est trouvée, soit il n'y a plus
d'environnement parent.  Si la propriété est trouvée, elle est retournée, sinon
il y aura une erreur à l'exécution.  Dans cet exemple, l'environnement global
est en haut à gauche du diagramme (il contient ~m~).

Avant d'exécuter le code de l'exemple, l'environnement global est vide.  Lorsque
la FIA est appelée, elle crée l'environnement en {{{color(c1)}}}.  Puisque la
FIA est définie à la racine, l'environnement parent est l'objet global.  À
...
fonction est définie, un objet fonction est créé qui contient son code et son
...
1. La seule façon que du code extérieur au module a d'accéder aux définitions
...
Dans ce diagramme et les suivants, on n'illustre pas les objets d'activation des
...
~mkG~.  Le diagramme ci-contre illustre cette différence par l'apparition d'un
nouvel objet d'activation pour ~mkG~ qui est référencé comme environnement de
~g~.  On a ~m.g.env~ qui pointe vers l'environnement créé par ~mkG~
({{{color(c1)}}}), et non l'environnement créé par la FIA ({{{color(c2)}}}).  On
voit alors que la propriété ~env~ d'une fonction n'est pas garantie de pointer
vers l'environnement dont nous avons besoin.
...
Le diagramme ci-contre illustre clairement le problème: la nouvelle fonction ~f~
...
figure ci-contre.  La redéfinition de ~a~ à ~2~ est enregistrée par
...
#+BEGIN_aside
Rappel: ~with~ a déjà été présentée {{{ref(hello-with, précédemment)}}}.
#+END_aside
...
liaison (qui associe ~a~ à 42), tandis que ~f.env~ pointe vers l'environnement
(la boîte au fond {{{color(c0)}}} autour de l'objet de liaison).
#+END_side-figure
...
l'objet de liaison.  Et l'objet de liaison peut être mutable: on peut y ajouter
ou y supprimer des propriétés.  Donc, on dira que la fonction ~f~ /capture
statiquement un environnement mutable/.  L'environnement ne peut être capturé
qu'à la définition de ~f~, mais cet environnement peut être modifié par la
suite.  Les deux diagrammes précédents illustrent clairement la différence de
...
liaison sont deux objets distincts, alors que dans le modèle ils ne sont qu'un
...
est bien capturée par la nouvelle fonction ~f~.  Tout cela est illustré
ci-contre.  La propriété ~m.E~ pointe sur l'objet de liaison ({{{color(c1)}}}),
