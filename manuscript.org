 ...
 - [[#travaux-connexes][Travaux connexes]]
   ...
   - [[#unstable-travaux-connexes-concernant-linstrumentation][UNSTABLE Travaux connexes concernant l'instrumentation]]
     ...
     - [[#bare-web-browsers][BARE Web browsers]]
       - [[#mechanisms-for-extension-4][Mechanisms for extension]]
     - [[#translate-lua][TRANSLATE Lua]]
        - [[#mechanisms-for-extension-5][Mechanisms for extension]]
...
*** BARE Smalltak
#+BEGIN_epigraph
Classification is the objectification of /ness/ness. — [[cite:Ing-81][Ing-81]]
#+END_epigraph
...
ordinateur intuitif, et qui peut servir pour tout usage.  Programmer ce système
serait simple comme bonjour, et c'est pourquoi il nomme son langage de
...
instances, et le mécanisme d'héritage.  De LISP, il emprunte le style de
...
La programmation par objets en Smalltalk repose sur six grands principes:

1. Tout est /objet/.
2. Les objets communiquent par envoi et réception de /messages/ (qui contiennent
   des objets).
3. Les objets ont leur propre mémoire (qui contient des objets)
...
Smalltalk is a language "for children of all ages".  As such, it must be simple
and powerful.  The classes and instances of Simula provide a good structure
metaphor.  Message-passing is simple and general enough to describe
/processing/.

Also, personal computing.

Object-oriented is opposed to function-oriented: ~o + 4~ means send the message
~+4~ to ~o~.  If ~o~ is the integer 3, we get 7.  If ~o~ is the string ~Three~,
we get ~Three4~.  The meaning of the expression depends on the objects involved:
this is dynamic dispatch, a key ingredient to manage complexity.
...
Smalltalk is more than a language: it's a system.  A reactive system.  The
language is only one piece of the computer, designed to be as friendly and
...
object, and global recompilation is avoided.  Thus, message-passing allows
modular recompilation.

Traits are mentionned, but not detailed.  [[cite:CBL+82][CBL+82]] has more on that.

#+BEGIN_QUOTE
As the class hierarchy grew, we began to notice that the constraint of
/pure-tree/ class hierarchies was causing code to become contorted, and that
generalizing the concept of "class hierarchy" to include directed acyclic graphs
would allow code to be organized more cleanly.
#+END_QUOTE

SIMULA introduced specialization.  Traits generalize the model:

#+BEGIN_QUOTE
SIMULA-67 noted that often an object is a /specialization/ of another, being
able to do the job of the first—and more.  It provided a means of expressing the
common portion once, in order that the specialized object need only specify the
way in which it was different from the simpler one.  The specialized object
/inherited/ the properties of the simpler one.

The Traits model notes that an objet (type) may be a synthesis of several
component abstractions, being abe to do the job if its components and more.  It
provides a means of expressing the common, or shared, parts once.
#+END_QUOTE

Sharing code has many benefits:

#+BEGIN_QUOTE
clarity of code through factoring or abstraction; uniformity of behavior,
including correctness; ease of maintenance; reduced swapping.
#+END_QUOTE

This surprisingly matches my understanding of traits closely.  Traits declare
method signatures and even state, and provide default implementations.  Compound
traits fuse multiple traits and can provide alternative implementations.
Classes can implement multiple traits, to reuse more code than by just subclassing.

The paper has an interesting, very high-level approach to the situation.
Instances have operations (methods), that have a specification (signature) and
an implementation (body).  Operations are equivalent if they have the same
specification.

Instances can be partitioned into equivalence classes based on their operation
sets.  If they have the same operations, they are the same type.  But there are
finer differences:

- One type may be strictly larger than another. (extension)
- Two types may have equivalent operations, but different realizations. (variation)
- Specialization is extension + variation
- Type A has operations 1 and 2, B has 2 and 3, and C has 1 and 3. (union)


Smalltalk is high-level, so has no explicit memory management.  Though only
reference counting is done, so cycles can't be detected.  One has to manually
break cycles in objects that should be collected (the irony is not lost on the
author).

Speaking of message-passing optimizations for Integers:

#+BEGIN_QUOTE
It should be obvious that to carry the message sending metaphor down into
integer arithmetic will result in a slow system on any non message oriented
computer.  The implementor must cheat, but not get caught.
#+END_QUOTE

Concluding about Smalltalk:
...
Additional story on the vision of Smalltalk can be found in [[cite:Kay-93][Kay-93]].

Kay was inspired by Simula, Sketchpad, and his biology major:
...
He was taken by Douglas Engelbart's vision of "augmenting the human intellect",
via an interactive tool to explore "thought vectors in concept space".

Seeing a flat panel for the first time, he envisioned the Dynabook.

He saw an object-based system as "a syntax-directed interpreter of messages sent
to it".

About mounting the Learning Research Group and heading it:

#+BEGIN_QUOTE
Bill English took me under his wing and helped me start my group as I had always
been a lone wolf and had no idea how to do it.  One of his suggestions was that
I should make a budget.  I am afraid that I really did ask Bill, "What's a
budget?"  I remembered at Utah, in pre-Mansfield Amendment days, Dave Evens
saying to me as he went off on a trip to ARPA, "We're almost out of money.  Got
to go get some more."  That seemed about right to me.  They give you some
money.  You spend it to find out what to do next.  You run out.  They give you
some more.  And so on.  PARC never quite made it to that idyllic standard, but
for the first half-decade it came close.

I needed a group because I had finally realized that I did not have all the
temperaments required to completely finish an idea.  I called it the Learning
Research Group (LRG) to be as vague as possible about our charter.  I only hired
people who got stars in their eyes when they heard about the notebook computer
idea.  I did not like meetings: did not believe brainstorming could substitute
for cool sustained thought.  When anyone asked what to do, and I did not have a
strong idea, I would point at the notebook model and say, "Advance that."
#+END_QUOTE

Quip on the name "Smalltalk":

#+BEGIN_QUOTE
The name was also a reaction against the "IndoEuropean god theory" where systems
were named Zeus, Odin, and Thor, and hardly did anything.  I figured that
"Smalltalk" was so innocuous a label that if it ever did anything nice people
would be pleasantly surprised.
#+END_QUOTE

Simula had good structure, but not everything was an object, it broke the
beauty.

LISP was an inspiration, as Kay sought an essential beauty, a robust principle
that could help anyone create programs.

But LISP was symbolic.  And according to Piaget and Bruner, children start
dealing with symbols at a later stage.  So Kay wanted a system where one could
manipulate icons.
...
Kay feels OOP is more than just ADT:

#+BEGIN_QUOTE
Even the way we taught children reflected this way of looking at objects.  Not
too surprisingly, this approach has considerable bearing on the ease of
programming, the size of the code needed, the integrity of the design, and so
on.  It is unfortunate that much of what is called "object-oriented programming"
today is simply old style programming with fancier constructs.  Many programs
are loaded with "assignment-style" operations now done by more expensive
attached procedures.
#+END_QUOTE

OOP techniques are no tied to any language:

#+BEGIN_QUOTE
Four techniques used together—persistent state, polymorphism, instantiation, and
methods-as-goals for the object—accoun for much of the power.  None of these
requires an "object-oriented language" to be employed—ALGOL 68 can almost be
turned to this style—an OOPL merely focuses the designer's mind in a particular,
fruitful direction.  However, doing encapsulation right is a commitment not just
to abstraction of state, but to eliminate state-oriented metaphors from
programming.
#+END_QUOTE

The key of OOP though is to avoid explicitly dealing with state.  Assignment
statements are low-level.  OOP is late-binding.

Kay was surprised by metaclasses, which for him were really just a way to
simplify instance initialization.  Instead, they gelled into a holly concept:

#+BEGIN_QUOTE
My guess is that Smalltalk had moved into the final phase I mentioned at the
beginning of this story, in which a way of doing things finally gets canonized
into an inflexible belief structure.
#+END_QUOTE

Hardware hampers progress in software.  Later-binding is always sought after in
software, but hardware pulls it down:

#+BEGIN_QUOTE
One way to think about progress in software is that a lot of it has been about
finding ways to /late-bind/, then waging campaigns to convince manufacturers to
build the ideas into hardware.  Early hardware had wired programs and
parameters; random access memory was a scheme to late-bind them.  Looping and
indexing used to be done by address modification in storage; index registers
were a way to late-bind.  Over the years software designers have found ways to
late-bind the locations of computations—this led to base/bounds registers,
segment relocation, paging MMUs, migratory processes, and so forth.
Time-sharing was held back for years because it was "inefficient"—but the
manufacturers would not put MMUs on the machines; universities had to it
themselves!  Recursion late-binds parameters to procedures, but it took years to
get even rudimentary stack mechanisms into CPUs.  Most machines still have no
support for dynamic allocation and garbage collection, and so forth.  In short,
most hardware designs today are just re-optimizations of moribund architectures.
#+END_QUOTE

OOP is late-binding:

#+BEGIN_QUOTE
OOP can be viewed as a comprehensive technique for late-binding as many things
as possible: the /mix/ of state and process in a set of behaviors, /where/ they
are locate, /what/ they are called, /when/ and /why/ they are invoked, /which/
HW is used, and so on, and more subtle, the strategies used in the OOP scheme
itself.
#+END_QUOTE

Metaobjects are late-binding.

Inverse vandalism is hampering the advancement of human intellect:

#+BEGIN_QUOTE
A twentieth century problem is that technology had become too "easy".  When it
was hard to do /anything/, whether good or bad, enough time was taken so that
the result was usually good.  Now we can make things almost trivially,
...
standard and goad.  Then the trick is to decouple the dissatisfaction from
self-worth—otherwise it is either too depressing or one stops too soon with
trivial results.
#+END_QUOTE

Late-binding in Smalltalk is that, when sending a message, everything after the
receiver is a token.  It is evaluated by the receiver, not the sender.

[[cite:Ing-81][Ing-81]] is a nice overview of Smalltalk.  A point that's worth stressing is that
Smalltalk was envisonied to be a standalone system with a single metaphor to
deal with the computer, from systems programming to creating music.  The VPRI
experiment was a re-telling for contemporary machines, and that shows what Kay
wants to build.

An ill-chosen prediction:

#+BEGIN_QUOTE
Natural Selection: Languages and systems that are of sound design will persist,
...
#+ATTR_HTML: :style margin-top:-7rem
#+BEGIN_side-figure
#+CAPTION: Un programme ressemble davantage à un réseau qu'à un arbre.
[[file:img/knuth0.svg]]
#+END_side-figure
...
#+ATTR_HTML: :style margin-top:-15rem
#+BEGIN_side-figure
#+CAPTION: À partir d'un programme littéraire WEB, WEAVE produit un document
#+CAPTION: imprimable, et TANGLE produit l'exécutable.
[[file:img/knuth1.svg]]
#+END_side-figure
...
#+BEGIN_side-figure
#+CAPTION: Les similarités d'un projet peuvent être récupérées par des outils
#+CAPTION: standard.  Ici, ~grep~ permet de créer différentes vues du projet.
...
#+CAPTION: de la propriété ~a~ commence par l'environnement le plus récent
#+CAPTION: (en bas), et retourne la valeur de la première cellule qui contient
#+CAPTION: une propriété "a"; ici c'est le second objet.  La recherche de ~g~
#+CAPTION: échoue.  Notons que le ~f~ du second objet est caché par le ~f~ de
#+CAPTION: l'objet en tête de la chaîne.
[[file:img/dls6.svg]]
#+END_side-figure
...
#+CAPTION: que l'environnement est distinct du l'objet de liaison: ~o~ pointe
#+CAPTION: vers l'objet de liaison, tandis que ~f.env~ pointe vers
$+CAPTION: l'environnement.
[[file:img/dls9.svg]]
#+END_side-figure
