...
recèlent d'exemples à suivre pour mystifier tout collaborateur.  Le standard
Unicode offre également de nouvelles opportunités (voir, par exemple, Mimic
[[cite:Mimic][Mimic]]).
#+END_aside
...
#+BEGIN_aside
On improvise plus difficilement un programme qu'un morceau de guitare.
Il existe cependant des environnements de programmation qui facilitent
...
C'est qu'il y
a une autre facette à un programme: sa /spécification/.  Ici, la
spécification est: "afficher le onzième nombre de la suite de Fibonacci".  Les
trois programmes sont équivalents car ils obéissent Les trois programmes
obéissent donc à cette spécification.  processus pour calculer une /fonction
mathématique/.

specification is not complete (otherwise it would be the program)

source code is deterministic: code -> processus -> result

but the task of the programmer is not:

result <- multiple processes <- multiple programs

there's a whole design space for achieving the goal

tool chain, programming languages, paradigms, algorithms, names, ...

the programmer is satisfîmes multiple goals: correctness first, but also
efficiency, and readability, maintainability...

that's where we come in: we explore the design space, and report on the
strengths and weaknesses of techniques for given applications.
...
interpreter is like a compiler, but executes the code immediately instead of
producing a binary.
