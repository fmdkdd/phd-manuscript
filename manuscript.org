...
* Contents                                                   :TOC@4:noexport:
 ...
 - [[#unstable-État-de-lart][UNSTABLE État de l'art]]
   - [[#unstable-une-chronologie-de-la-modularité-dans-les-langages-de-programmation][UNSTABLE Une chronologie de la modularité dans les langages de programmation]]
     ...
     - [[#translate-la-programmation-par-aspects][TRANSLATE La programmation par aspects]]
...
*** TRANSLATE La programmation par aspects
...
d'implémentation d'un système [[cite:KLM+97][KLM+97]].
...
Pour expliquer ce qu'est un aspect, part d'un exemple [[cite:MKL-97][MKL-97]].  Le système qui
...
Pour satisfaire la première contrainte, il suffit de créer des filtres
composables.  D'abord des filtres basiques, comme ~Or~, ~And~, ~Not~, analogues
des opérateurs communs sur les bits, mais appliqués aux images, pixel par pixel:
...
filtres précédents.  Le code est clair, simple et direct, donc facile à écrire
...
créerait qu'une seule image, le résultat.  Mais il faut écrire cette version
optimisée directement, et on ne bénéficie plus alors de la composition des
filtres élémentaires.  Cherchant l'efficacité, les auteurs ont réécrit tous les
filtres complexes directement.  Le système efficace comporte trois optimisations
majeures: la fusion de boucle, l'utilisation d'un bassin d'objet pour éviter
l'allocation d'images intermédiaires, et la mémoisation des résultats
intermédiaires.  Le système efficace comporte 35213 lignes de code, alors que la
version simple, mais non efficace, en comporte 768; 5 fois moins de code à
écrire, et à maintenir.  C'est d'autant plus dommage que les versions optimisées
suivent toujours le même motif: fusionner les boucles, pour combiner les
opérations à l'intérieur des boucles plutôt qu'à l'extérieur.
...
Les auteurs distinguent alors les /composants/ des /aspects/.  Si une
fonctionnalité du système peut être implémentée par une procédure (ou une
...
ce langage.  Pour une application écrite suivant la programmation par aspects,
on distingue: le programme des composants, le (ou les) programme des aspects, le
...
boucle, la mémoisation, et le bassin d'objets sont trois programmes d'aspects
différents.  La fusion de boucle par exemple teste si les arguments d'un filtre
ont une itération compatible avec l'itération faite par le filtre; si c'est le
cas, on peut générer une seule boucle qui applique les opérations des arguments
et du filtre:
...
Enfin, dans ce système, le tisseur génère du code C pour les filtres.
...
# Aspect languages is the important contribution of the original paper, but
# AspectJ is just /one/ generalized language, not the original implementation.

# Lamping [[cite:Lam-97][Lam-97]] nails it: aspects are interesting /because/ they interact with
# components.  They are /not/ components; they are pervasive issues.

# Wolfgang [[cite:Meu-97][Meu-97]] links AOP to Monads.  Monads can be seen as encapsulating code
# for one concern

# Reification of language constructs
# Implicit vs explicit extension
# COMEFROM vs GOTO
# Duality, other side of the coin
