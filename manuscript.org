...
lorsqu'elle exécute le code source.  On peut considérer que le code décrit un
ordre, et la machine exécute cet ordre.  Quand on parle de /programme/, on
désigne à la fois le code source et le processus.

Du point de vue de la machine, un programme est une simple liste d'instructions.
Une séquence d'additions, de soustractions, d'écritures et lectures mémoire, et
de sauts conditionnels.  Ces instructions sont présentées à la machine sous la
seule forme que son processeur est capable de manipuler: le binaire, une suite
de zéros et de uns.
...
humain; il lui faut des repères, des unités plus digestibles par nos facultés
organiques.  Un programmeur manipule donc rarement des bits afin de déclarer ses
...
affiche le onzième nombre de la suite de Fibonacci, sans être familier avec le
langage de programmation utilisé.  Les mots /évoquent/ immédiatement du sens
alors que les nombres doivent être décodés.
...
contenu de ce document de thèse à des non-francophones, soit je rédige une
...
possibles, les deux situations sont également improbables.
#+END_aside

Le /compilateur/ traduit les programmes d'un langage de programmation vers le
langage de la machine.  Le compilateur lit le texte du code source, vérifie que
ce texte est conforme aux règles syntaxiques du langage qu'il traduit, puis
applique les règles de traduction en langage machine.  Un compilateur permet
donc au programmeur d'obtenir un programme exécutable par la machine à partir de
code écrit dans le langage de son choix.  Le programmeur peut manipuler des
mots, et la machine du binaire.

Comme le compilateur, l'/interpréteur/ traduit le code source, mais plutôt que
de produire un fichier exécutable, l'interpréteur exécute directement le
...
ce sont toutes des façons d'exécuter le code source.  Et si l'on exécute [[fib]], on
obtient:
...
le programmeur.  On aurait pu écrire le programme de [[fib]] ainsi:
...
offre également de nouvelles opportunités (voir, par exemple, Mimic [[cite:Mimic][Mimic]]).
#+END_aside
...
supplémentaires le troisième programme requiert, ni le nombre de registres qu'il
utilise, ni le nombre de défauts de cache qu'il cause.  On cherche à comparer le
/résultat/ du programme observable par l'utilisateur.  Ici, quand on exécute
chacun des trois programmes, l'interpréteur JavaScript affiche 89; les
programmes ont donc le même résultat.
...
seconde en voiture.  En revanche, leur mode de transport nous intéresse quand il
s'agit de comparer leurs émissions carbones.
#+END_aside
...
utilisée.  L'efficacité du programme c'est /comment/ le programme calcule son
résultat; la machine peut avoir une grande influence sur l'efficacité.

#+BEGIN_aside
On improvise plus difficilement un programme qu'un morceau de guitare, bien
qu'il existe aujourd'hui des environnements de programmation qui facilitent
l'improvisation [[cite:sonicpi][sonicpi]] [[cite:BFd+13][BFd+13]].
#+END_aside

Enfin, il y a le résultat obtenu, et il y a le résultat attendu.  Le programmeur
élabore le programme avec une idée en tête de son résultat, une intention.
C'est cette idée qui guide le programmeur dans l'élaboration du programme: il
construit le code source afin d'engendrer un processus qui produira le résultat
attendu.  Ce résultat attendu est appelé la /spécification/ du programme.  Cette
spécification peut être plus ou moins détaillée; en voici trois:
...
La deuxième spécification est plus précise que la première, et la troisième
plus précise que la deuxième.  Un programme qui obéit à la troisième
spécification obéira donc aussi à la deuxième et à la première.  Notons que les
deux premières spécifications s'intéressent strictement au résultat du
programme, alors que la troisième stipule aussi sa structure: le programme doit
utiliser une fonction récursive.  On pourrait tout aussi bien spécifier
l'efficacité du programme: il doit s'exécuter en moins d'une seconde sur telle
machine, etc.  La spécification décrit la fonctionnalité principale du programme.
...
pas détailler le processus de manière exhaustive, car si c'était le cas, ce
serait le programme.  Et c'est précisément en dehors de la spécification
qu'opère la créativité du programmeur.

En effet, l'élaboration du programme à partir d'une spécification a de nombreux
degrés de liberté.  Il suffit de voir qu'il y une infinité de programmes qui
peuvent satisfaire une spécification.  Tout comme il y a une infinité de façons
d'obtenir le nombre 1: 1, 2 - 1, 1 + 0, 56/56, ...  Tout ce que la spécification
ne précise pas est laissé au choix du programmeur.  Il y a donc une infinité de
processus qui obéissent à une spécification donnée, et une infinité de
programmes qui engendrent ces processus.

L'expertise du programmeur est de savoir exactement quels processus choisir, et
quels programmes écrire, parmi ces infinités.  Pour chaque programme, il faut
choisir le langage de programmation, le compilateur, les bibliothèques, les
algorithmes, les structures de données, etc.  Tous ces éléments doivent
s'accorder pour engendrer le résultat attendu.

Mais le programme n'a pas seule vocation a être exécuté.  Un programme est
d'abord écrit, puis lu, puis corrigé, puis étendu.  Lors de son développement,
de nombreuses paires d'yeux vont le scruter et le modifier.  Le programmeur doit
communiquer son intention non plus seulement à la machine, mais aussi à ses
collègues.

Et chaque humain a ses préférences particulières.  Le choix des noms de
variables et de fonctions est important pour communiquer leur fonction dans le
programme.  La documentation est cruciale, car le code source ne peut exprimer à
lui seul tous les choix faits par le programmeur.

Le programmeur doit satisfaire plusieurs buts.  Avant tout, la correction du
programme par rapport à la spécification.  Ensuite, les priorités dépendent de
l'application.  L'efficacité du programme est toujours pertinente.  Mais on peut
aussi souhaiter que le programme soit lisible, maintenable, ou modulaire.

C'est là que nous intervenons: nous explorons l'espace des programmes possibles,
et rapportons les avantages et inconvénients de chaque techniquse pour une
application donnée.
...
La définition exacte de cette fonction correspond à la spécification de
...
programmes différents: le programme d'origine, ou le programme qui contient les
modifications.  On retrouve alors les deux processus attendus, mais les
similarités de la spécification et du code source sont factorisées.

L'intérêt de cette seconde représentation factorisée, c'est qu'elle préserve la
séparation des préoccupations: l'interpréteur d'origine est conservé, et
toujours présent dans le code source.  La modification est séparée, et non plus
mêlée dans le code de l'interpréteur.
...
dupliquer le code de l'interpréteur quatre fois, mais juste à décrire les
quatre modifications requises par les analyses.

Cette factorisation correspond directement à une factorisation algébrique.  Si
I_1 est l'interpréteur d'origine et I_2 l'interpréteur modifié, on note M_2 la
modification qui transforme I_1 en I_2 (M_2 est une fonction).  On a I_2 = M_2 I_1.
Soient deux autres interpréteurs: I_3 et I_4, et leurs modification respectives M_3
et M_4.  On a:
...
(I_1, I_2, I_3, I_4) = (Id I_1, M_2 I_2, M_3 I_3, M_4 I_4) = (Id, M_2, M_3, M_4) I_1
...
Mais si intuitivement une telle factorisation semble possible, la question
naturelle qui suit est: comment réaliser cette factorisation dans le code
...
En observant le diff de l'instrumentation plus près, on peut distinguer quatre
catégories de changements: les imports/exports, l'ajout du paramètre /program
counter/, séparer l'évaluation des valeurs à facettes, et les ajouts à l'objet
~global~.
...
contexte, le sens de la phrase peut avoir des sens bien différents.
...
comportement.  Le contexte du programme est donc définit par toutes les couches
...
sans avoir à toucher le code source du programme.  Le détournement permet la
séparation des préoccupations.
