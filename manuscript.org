...
déléguée à une fonction nommée ~eval-TERM~.  La fonction principale ~eval-run~
ne fait que construire le nom de cette fonction pour l'invoquer sur le nœud de
l'arbre syntaxique passé en paramètre.

Toutes ces définitions sont faites à l'intérieur d'une fonction ~make-interp~,
dans l'intention de répliquer la structure de Narcissus.  Dans Narcissus, les
fonctions d'interprétation sont internes, et la seule fonction exportée est
~eval~ pour évaluer des programmes.  Un programme qui importe Narcissus n'a pas
de référence aux fonctions internes de l'interpréteur.  En Elisp, les fonctions
définies par ~defun~ sont globales, donc seront accessibles en dehors.  Mais
dans l'esprit de répliquer le comportement de Narcissus, on ne fera comme si ces
références étaient inaccessibles.  La seule fonction exportée est donc
~eval-run~, que l'on capture dans la variable ~interp~.

[actually need to go back to an object rules that contains lambdas, because
function cells are not dynamically scoped.  Using flet is cheating because it
just temporarily swaps a global variable]
