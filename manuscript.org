...
Un interpréteur peut définir plusieurs opérations.  Wadler donne l'exemple d'un
/pretty-printer/, une opération d'affichage des arbres syntaxiques dont la
sortie est plus amicale pour l'œil humain.  Pour afficher les expressions
arithmétiques, on pourrait transformer le ~Plus~ préfixe en notation ~+~ infixe,
et faire disparaître le mot ~Constant~ qui n'apporte pas d'information capitale.

#+BEGIN_SRC haskell
print :: Term -> String
print (Constant n) = show n
print (Plus t1 t2) = print t1 ++ "+" ++ print t2
print (Mult t1 t2) = print t1 ++ "*" ++ print t2
#+END_SRC
...
[now, what does adding a term entails?]

[what does adding an operation entails?]

[type safety is not necessary for it to be a problem, but it makes it harder]
