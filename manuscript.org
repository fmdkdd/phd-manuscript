...
* Contents                                                   :TOC@4:noexport:
 ...
 - [[#contexte][Contexte]]
   ...
   - [[#stable-modéliser-le-monde-la-programmation-objet][STABLE Modéliser le monde: la programmation objet]]
     - [[#classes-et-héritage-smalltak][Classes et héritage: Smalltak]]
...
** STABLE Modéliser le monde: la programmation objet
#+BEGIN_full-figure
#+CAPTION: L'École d'Athènes. Raphaël, 1509--1510.
[[file:img/scuola.jpg]]
#+END_full-figure

*** Classes et héritage: Smalltak
:PROPERTIES:
:CUSTOM_ID: Smalltalk
:END:
#+BEGIN_epigraph
...
ordinateur intuitif, qui pourra servir aux enfants "de tout age".  Programmer ce
...
#+BEGIN_side-figure
#+CAPTION: L'héritage permet aux classes ~Integer~ et ~Float~ de réutiliser
#+CAPTION: les méthodes de ~Number~.
...
Que le langage soit centré sur les objets a un avantage pour l'étendre.  Ingalls
donne l'exemple d'ajouter un nouvel objet au système, et de faire les
...
Un autre avantage que les objets ont de ne pas dépendre de la représentation
interne d'autres objets, c'est la recompilation séparée.  Modifier ou ajouter un
champ à la classe ~Rectangle~ n'impacte que le code de l'objet, jamais le code
extérieur.  Les objets peuvent être compilés indépendamment.
...
#+BEGIN_side-figure
#+CAPTION: Ces trois classes ont toutes deux à deux des méthodes en commun, mais
#+CAPTION: leur intersection est nulle.  Elles ne peuvent pas être factorisées
#+CAPTION: par l'héritage.
...
#+BEGIN_side-figure
#+CAPTION: Une classe peut récupérer les méthodes de plusieurs traits à la fois,
#+CAPTION: ce qui permet de factoriser ces trois classes.
...
classe ~Number~, on fait référence au message ~-~, qui n'est pas définit sur
...
formes, qui sont leur manifestation concrète et imparfaites.  Platon considère
que les formes sont plus réelles que leurs instances.  En programmation objet,
c'est le point de vue que les classes sont plus réelles que les objets.
Aristote croit qu'il est possible d'établir l'unique taxonomie correcte de
toutes les choses naturelles.  Il classe une chose à partir de ses propriétés
essentielles (le genre), et des propriétés qui la distingue des autres choses du
même genre (le différentiel).  On voit le parallèle avec l'héritage en
programmation par objets: quand une classe hérite, elle déclare son genre, et
les méthodes additionnelles décrivent son différentiel.
...
#+BEGIN_side-figure
#+CAPTION: Dans des langages à classes, on réifie le concept d'éléphant
#+CAPTION: (et même des sous-concept) avant de pouvoir manipuler les objets
#+CAPTION: qui représentent Clyde et Fred.  Les deux mécanismes de partage
#+CAPTION: sont l'héritage et l'instantiation.
[[file:img/self0.svg]]
#+END_side-figure

En utilisant les prototypes, je ne crée pas de concepts.  Si Clyde est le
...
a Fred, il a en a autant que Clyde, son prototype.  L'information non spécifique
à Fred est sauvegardée par le prototype: c'est le mécanisme de /délégation/.

#+ATTR_HTML: :style margin-top:-4rem
#+BEGIN_side-figure
#+CAPTION: Dans les langages à prototype, on manipule directement les
#+CAPTION: objets qui représentent Clyde et Fred.  Le mécanisme essentiel
#+CAPTION: de partage est la délégation.
[[file:img/self1.svg]]
#+END_side-figure
...
objets qui ont un comportement unique.  Mais il arrive souvent que l'on doivent
gérer de nombreux objets similaires.  Les concepteurs de Self décrivent une
façon de structurer ces objets à l'aide de /traits/ [[cite:UCC+91][UCC+91]].  Un trait est un
objet qui regroupe les méthodes utilisées par plusieurs objets.  On crée ensuite
un objet prototype qui étend ce trait, et fournit une implémentation et des
valeurs d'attributs par défaut.  Tous les objets de la même famille étendent ce
...
L'idée est proche des traits en Smalltalk, sauf qu'en Smalltalk le langage fait
une distinction syntaxique entre classes, traits, et instances, alors qu'en Self
ce sont tous des objets qui ont juste des rôles différents.
...
La programmation par objets nous permet réifier les acteurs du domaine
d'application.  Dans la simulation d'un écosystème qui comprend de l'herbe, des
moutons et des loups, on trouve des objets qui représentent l'herbe, des objets
moutons, et des objets loups.  Mais un programme peut réifier les acteurs de sa
propre exécution, les objets de la sémantique; on parle alors de /réflexion/.
La réflexion permet à un programme d'altérer sa structure, ou de modifier la
sémantique du langage, pendant qu'il s'exécute.  Cette adaptation dynamique
offre de nouvelles façons de concevoir des programmes modulaires.

En Java, un programme peut obtenir les méthodes d'un objet via l'API de
réflexion:

#+BEGIN_SRC java :classname A :results verbatim :exports both :eval no-export
...
la programmation par objets: on a les classes ~Field~, ~Method~, ~Object~ et
~Class~.  Non seulement on peut récupérer ces

Le protocole metaobjet de CommonLisp va plus loin: il permet de modifier la
façon dont le langage interprète le passage de message ou l'héritage [[cite:KRB-91][KRB-91]].  [exemple
