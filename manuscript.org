...
* Contents                                                   :TOC@4:noexport:
 ...
 - [[#appendices][Appendices]]
   - [[#construire-un-interpréteur-par-modules-1][Construire un interpréteur par modules]]
     ...
     - [[#la-différence-subtile][La différence subtile]]
...
// Oh but wait, now we have a type problem if we are not careful
...
:END:
Utiliser plusieurs ~with~ c'est composer les environnements.  Composer les
fonctions, c'est retourner un objet unique qui sert d'environnement.  Le
'fallback' fourni implicitement par le mécanisme d'environnements doit être
émulé par la composition de fonctions avec ~__proto__~.

C'est assez clair avec un diagramme.
