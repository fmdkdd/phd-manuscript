...
#+ATTR_LATEX: :height 3.5cm :float t
...
On constate également que l’instrumentation *duplique tout le code* de
...
1. De nouvelles définitions ont besoin d’être importées dans le module de
   l’interpréteur, et une nouvelle fonction est exportée.  Ce sont de simples
   ajouts qui sont localisés en début et en fin de fichier respectivement.
   Voici comment ils se présentent dans le code:
...
2. Les changements effectués pour accommoder le /program counter/ utilisé par
...
3. Les changements effectués dans l’exécution de l’arbre de syntaxe abstrait
...
4. Les changements effectués sur l’environnement d’exécution de code client.
...
catégories indique qu’il y a un potentiel de factorisation de ces changements.
