...
~plus~ ligne [[(ref:foal-ex2)]].  On voit qu'on peut appliquer la nouvelle opération
...
prend un module de base et retourne un module enrichit avec l'opération ~show~.
Les modules ici sont de simples objets JavaScript.  L'objet ~base~ passé en
...
retourne un nouveau module (objet) qui contient les versions enrichies de ~num~
et ~plus~ (ligne [[(foal-ex7)]]).
...
affectés par l'ajout de l'opération ~show~.
...
Le premier argument de ~s.plus.new~ est un ~num~ et le second un ~s.num~.
Si on appelle l'opération ~show~ sur cette expression, le programme lève une
erreur car l'opération ~show~ n'est pas définie sur ~num~.  C'est une erreur
qu'un langage typé statiquement pourrait capturer: la définition de ~plus.show~
requiert que ~this.l~ supporte également ~show~.  Donc, le constructeur de
termes ~s.plus~ ne devrait accepter que des termes du langage enrichi: des
~s.plus~ ou des ~s.num~.  Or, on ne modifie pas ici la définition du
constructeur pour restreindre les objets acceptés.

: s.plus.new : Term with Show -> Term with Show -> String

Pour y remédier, on peut empêcher la création d'expressions mixtes par
construction.  Il suffit de n'autoriser qu'un seul module de termes à la fois,
en masquant les références vers les ~num~ et ~plus~ originaux.  C'est possible
avec la construction ~with~ de JavaScript.
...
les noms des dérivés cachent les définitions initiales dans l'environnement.
Par construction, les expressions créées à l'intérieur du ~with~ ne permettent
pas de mixer les variantes du langage.
...
afficher l'experssion ~num(1)~.  On boucle ce module en lui passant ~show()~
comme argument à la ligne [[(foal-ex12)]].

La construction ~with~ et une fonction ont en commun de créer un environnement
lexical pour le code de leur bloc.  Pour empêcher la création d'expressions
mixtes, il suffit donc de contrôler les noms qui sont accessibles dans le bloc.
Ici, on restreint les expressions au seul terme ~num~ du module ~show~.
