...
L’encapsulation est le critère de décomposition en module que Parnas trouve le
plus efficace.  En appliquant ce critère, Parnas conseille notamment de mettre
dans un même module une structure de données et ses procédures d’accès et de
modification.  Une notion de type abstrait de donnée qu’il ne nomme pas comme
telle.
...
les étapes de traitement.  Un interpréteur et un compilateur en partie résolvent
...
Mais une décomposition dont les modules adhèrent strictement à l’encapsulation
peut

# Performance issues arise because encapsulation is costly.  Should use a mapper
# to efficient code.
