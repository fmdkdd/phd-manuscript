 ...
 - [[#unstable-État-de-lart][UNSTABLE État de l’art]]
   - [[#unstable-une-chronologie-de-la-modularité-dans-les-langages-de-programmation][UNSTABLE Une chronologie de la modularité dans les langages de programmation]]
     ...
     - [[#translate-self][TRANSLATE Self]]
       - [[#mechanisms-for-extension][Mechanisms for extension]]
     - [[#translate-limplémentation-ouverte][TRANSLATE L’implémentation ouverte]]
...
*** TRANSLATE L’implémentation ouverte
...
est donc un interprète.  Donc, la réflexion dans les langages de programmation
n’est qu’un cas particulier de réflexion, qu’on peut appliquer à tout système
qui expose une interface.  Et comme un interprète réflexif permet au programme
d’inspecter et de modifier le processus engendré par l’interprète, il en va de
même pour un système plus général.

Rao définit un système à implémentation ouverte comme un système ayant deux
interfaces: l’interface /de base/, qui est l’interface classique associée au
système, et l’interface de /meta-niveau/, qui révèle en partie l’implémentation
de l’interface de base.

Le papier donne un exemple d’un système de gestion de fenêtre pour CLOS, appelé
Silica.  Dans un système de gestion de fenêtre, l’interface de base est la
construction d’arbres de fenêtres, d’interroger et de modifier ces arbres (pour
accéder aux fenêtres, en ajouter, les retirer...), et de lire et écrire dans les
fenêtres.  Comment les fenêtres, et la structure d’arbre sont implémentés peut
impacter l’utilisateur du système de fenêtre.

En particulier, Rao donne l’exemple d’un tableur qui créé une fenêtre pour
chaque cellule de la feuille de calcul.  Ce choix peut paraître curieux, mais la
justification est de réutiliser la fonctionnalité du système de fenêtre (capture
de la souris, peindre les cellules en fonction de leur visibilité, ...) pour
simplifier l’implémentation du tableur.  Mais dans un système de fenêtre naïf,
il faut allouer de la mémoire pour chaque fenêtre, même si dans le cas du
tableur, on pourrait partager bon nombre de propriétés entre les fenêtres
utilisées comme cellules.  De plus, lorsque qu’un clic de souris est détecté par
le système de fenêtre, il faut parcourir les arbres de fenêtres pour déterminer
la fenêtre qui est actuellement au dessus des autres, parce que les fenêtres
peuvent se superposer.  Mais dans un tableur, les cellules ne peuvent pas se
superposer, donc on peut déterminer le clic de souris directement grâce à ses
coordonnées.  Ces optimisations ne sont possibles que si le système de fenêtre
laisse au programme client la possibilité de modifier son implémentation.

Les langages à objets ont des avantages pour Silica: le paradigme objet
correspond bien au domaine d’application (ici, les fenêtres); le polymorphisme
permet à plusieurs implémentations de coexister et d’interagir; et l’héritage
permet de réutiliser et d’étendre du code existant.


La réflexion n’est en revanche qu’un mécanisme parmi d’autres pour
l’implémentation ouverte.  L’implémentation ouverte ne préconise que d’offrir
une interface supplémentaire pour avoir plus de contrôle sur l’implémentation du
système.  Que cette interface soit une interface réflexive est orthogonal.

[[cite:KLL+97][KLL+97]] donne des recettes pour construire des implémentations ouvertes, en
