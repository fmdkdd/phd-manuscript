...
** UNREAD En programmation littéraire
...
** UNREAD En programmation par objets
...
L'objet ~interp_modal~ est change de prototype suivant l'évaluation du
programme.  Au début, son prototype est ~interp_traceable~, puis après
l'évaluation d'une instruction ~start_trace~, son prototype devient
~interp_log~.  Dans cet état, évaluer l'instruction ~end_trace~ restaure le
prototype initial d'~interp_modal~.
#+END_side-figure
...
** UNREAD En programmation par aspects
...
#+BEGIN_SRC java
aspect NumDouble {
  pointcut eval(Num num): call(int eval() && target(num));
...
En compilant l'aspect ~NumDouble~, on obtient l'interpréteur modifié pour
lequel toutes les évaluations de termes ~Num~ ont une valeur doublée.

L'aspect est défini dans une classe à part entière, séparée du code de
l'interpréteur.  La modification du code est effectuée par le tisseur d'aspects,
lorsque l'on invoque la compilation de l'aspect par ~ajc~, le compilateur
d'AspectJ.  On peut aussi compiler le programme sans l'aspect, avec le
compilateur ~javac~, et on obtient l'interpréteur au comportement standard.
Deux compilateurs pour deux programmes, mais un seul code source.  La
programmation par aspects permet donc le détournement de cet interpréteur, mais
dans cet exemple le détournement est statique: il se fait à la compilation.

On peut répliquer l'exemple d'activation dynamique de la trace d'évaluation
...
#+BEGIN_aside
On fait du terme ~Trace~ une expression plutôt qu'une déclaration (/statement/)
comme dans l'exemple JavaScript, mais les deux formes sont réalisables en
...
l'étendue d'un appel à ~Trace.eval~.  La coupe ~cflow~ (cf. \sect[[#aspectj]]) inspecte
la pile d'appels, et on s'intéresse uniquement aux appels à ~eval~ qui sont
faits lorsque ~Trace.eval~ est déjà présente sur la pile.  Pour tous les appels
qui correspondent, on exécute l'affichage ligne [[(aj-trace)]] dans la méthode
d'aspect ~before~.  La sortie standard de l'évaluation du programme ~e2~ est
donc:
...
On peut intercepter les appels à ~f~, ou les lectures de ~a~ dans la fonction
~f~ en utilisant AspectScript ainsi:
...
#+BEGIN_aside
La section [[#why-no-aop]] contient davantage de détails sur le fonctionnement
d'AspectScript et ses inconvénients pour détourner Narcissus.
#+END_aside
...
par l'appelant.  Dans le Lisp intégré à Emacs (Elisp [[cite:Elisp][Elisp]]), la portée dynamique
...
sans avoir à modifier son code, de la détourner.  On peut facilement tirer
partie de ce mécanisme sur un interpréteur analogue à l'interpréteur JavaScript
précédent:
...
Un programme pour notre interpréteur sera donné sous forme de liste, la façon
canonique de représenter un arbre syntaxique en Lisp.  La fonction ~interp~
prend cet arbre syntaxique et retourne la valeur du programme, comme on peut le
voir à la fin de cet extrait.  On a toujours le même langage arithmétique
minimal construit des termes ~num~ et ~plus~.  L'évaluation de chaque terme est
déléguée à une lambda associée au type du terme dans la liste associative
~eval-rules~.  Cette liste est une variable que l'on va pouvoir modifier par
portée dynamique:
...
~eval-num~ est remplacée dynamiquement par ~eval-num-double~.  Mais en Emacs
Lisp on obtient 1, indiquant que la fonction ~eval-num~ n'a pas été changée par
le ~let~.  La cause de ce comportement est la séparation des espaces de noms
entre les fonctions et les valeurs; c'est la différence entre les dialectes
Lisp_1 et Lisp_2 décrite par Gabriel et Pitman [[cite:GP-88][GP-88]].  Dans un Lisp_1, l'espace de
noms est commun aux fonctions et aux valeurs, ce qui résoudrait le problème.
Sauf que les Lisp_1, comme Scheme ou Racket, n'ont pas de portée dynamique par
défaut: pour qu'une variable bénéficie de portée dynamique, il faut la déclarer
comme telle (un /parameter/ en Racket par exemple).

Le mécanisme de portée dynamique est le mécanisme plus simple pour détourner un
interpréteur, mais seulement quand il est proposé par le langage.  Or, peu de
...
dans plusieurs cas on a réussi à modifier le comportement de l'interpréteur sans
...
savoir quelles fonctions sont en charge de l'évaluation de tel terme, etc.
...
suivants, on va montrer comment répliquer une forme de portée dynamique en
utilisant des constructions JavaScript standards, afin de détourner un
...
#+BEGIN_fixme
FIXME: Make answers to research questions more proeminent; more precise.  Make
bolder distinction between "small" and "up there" perspectives.
#+END_fixme
...
des interpréteurs existants par détournement, en minimisant les changements de
...
Que faut-il en retenir?  Que le détournement est une façon viable de modifier
...
prototyper des analyses, des variantes d'interprétation, sans avoir à modifier
...
Cette construction, controversée, n'est soit n'est pas importante, c'est juste
le moyen le plus direct pour obtenir de l'indirection dans le cas présent.  Même
si la plupart des exemples de ce document ont utilisé le langage JavaScript, le
langage même importe peu, tant que des mécanismes d'indirection sont présents.
Pour détourner un programme, il faut introduire de l'indirection.  Et de façon
générale, ce principe vaut aussi pour étendre un programme.  Si les références
et les appels de méthodes sont indirects, ils seront plus faciles à changer.
...
améliorer la performance de ~with~.  Soit en optimisant cette construction dans
les moteurs d'exécution de JavaScript communs, ou bien en trouvant un mécanisme
d'indirection alternatif qui a moins d'impact sur le temps d'exécution.  Dans le
second cas, un gain potentiel serait d'évaluer partiellement le module détourné
et ses extensions, afin de déterminer les variables et fonctions qui sont
étendues, et de résoudre toutes les références en avance.  Cette transformation
pourrait même être faite par transformation de code, sans avoir à modifier les
moteurs d'exécution.  De façon générale, les mécanismes d'indirection posent
