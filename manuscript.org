...
* Contents                                                   :TOC@4:noexport:
 ...
 - [[#le-problème-étendre-des-interpréteurs][Le problème: étendre des interpréteurs]]
   - [[#unread-préliminaires-code-source-processus-et-spécification][UNREAD Préliminaires: code source, processus, et spécification]]
   - [[#unwritten-une-présentation-du-langage-javascript][UNWRITTEN Une présentation du langage JavaScript]]
   ...
   - [[#unread-Étude-de-cas-instrumentation-ad-hoc-de-narcissus][UNREAD Étude de cas: instrumentation ad-hoc de Narcissus]]
   - [[#unread-le-but-factoriser-les-changements][UNREAD Le but: factoriser les changements]]
   - [[#unread-quatre-axes-de-factorisation-pour-linstrumentation][UNREAD Quatre axes de factorisation pour l'instrumentation]]
   - [[#unread-refactoriser-narcissus-une-solution-coûteuse][UNREAD Refactoriser Narcissus: une solution coûteuse]]
 ...
 - [[#contexte][Contexte]]
   ...
   - [[#réifier-le-langage-pour-séparer-les-préoccupations-23][Réifier le langage pour séparer les préoccupations [2/3]]]
   ...
   - [[#tisser-les-facettes-dun-artefact-dinnombrables-dimensions-33][Tisser les facettes d'un artefact d'innombrables dimensions [3/3]]]
   ...
   - [[#la-modularité-du-programme-nest-pas-la-modularité-du-code-source-23][La modularité du programme n'est pas la modularité du code source [2/3]]]
 ...
 - [[#construire-un-interpréteur-par-modules-04][Construire un interpréteur par modules [0/4]]]
   - [[#unread-ajouter-des-termes][UNREAD Ajouter des termes]]
   - [[#unread-ajouter-des-opérations][UNREAD Ajouter des opérations]]
   - [[#unwritten-modifier-des-opérations][UNWRITTEN Modifier des opérations]]
   - [[#unwritten-passer-de-létat-aux-opérations][UNWRITTEN Passer de l'état aux opérations]]
...
:CUSTOM_ID: problem
:END:
** UNREAD Préliminaires: code source, processus, et spécification
:PROPERTIES:
:CUSTOM_ID: defs
:END:
...
** UNWRITTEN Une présentation du langage JavaScript
Puisque JavaScript est le langage de programmation le plus utilisé dans ce
document, des présentations sont de mise.

*** Genèse chez Netscape
#+BEGIN_aside
Plus de 90% des sites web sont visités par l'un des navigateurs suivant: Chrome,
Firefox, Safari, Internet Explorer ou Opera.  Tous permettent d'évaluer des
programmes JavaScript dans une page web.
#+END_aside
...
compatibles avec toutes les plates-formes, et surtout sont abandonnées par leurs
constructeurs.  JavaScript demeure le seul langage qui peut accompagner une page
web et être exécuté par tous ces navigateurs sans configuration supplémentaire.
...
d'une riche interface avec tous les éléments d'une page web, et plus
généralement de l'ordinateur: on peut facilement modifier l'arbre d'une page,
son style, mais on peut tout aussi aisément capturer le pointeur de la souris,
redimensionner la fenêtre du navigateur, ou accéder à la caméra de l'ordinateur.

Cependant, les programmes en JavaScript ne sont pas restreints aux navigateurs.
Les interpréteurs JavaScript comme V8 (de Chrome) ou SpiderMonkey (de Firefox)
sont exécutables en dehors du navigateur, et JavaScript devient alors comparable
à l'utilité faite du langage Python: un langage simple à prendre en mains,
disposant d'une riche collection de bibliothèques, et exécutable partout ou des
interpréteurs sont disponibles.

À l'origine le langage était destiné seulement aux navigateurs.  Son auteur,
Brendan Eich, est recruté par la compagnie Netscape dans le but de faire une
version du langage Scheme pour leur navigateur, le simplement nommé Netscape
Navigator.  En raison d'un partenariat avec Sun, le langage devait ressembler à
Java; de là aussi vient son nom.  Netscape souhaitait un langage de
programmation qui pouvait être inclus dans des les fichiers HTML des pages web,
un langage pour débutants, un langage pour des petits bouts de code, des
scripts, qui viendraient suppléer les applets Java.

En mai 1995, Brendan Eich produit le premier prototype du langage, qui
accompagnera la sortie de Netscape Navigator 2.0 en mars 1996.  En raison de son
succès, Microsoft implémente à son tour un interpréteur de JavaScript, JScript,
en août de la même année dans son navigateur Internet Explorer.  Les deux
versions cohabitent avec de légères différences, ce qui conduit à la
standardisation du langage sous le nom d'ECMAScript en juin 1997.

#+BEGIN_aside
Pour les conversions implicites tout azimuts, Brendan Eich blâme PHP.
#+END_aside

*** Un langage à prototypes qui doit ressembler à Java
Sous ses allures de langage impératif, JavaScript a plus en commun avec Self ou
Scheme qu'avec Java.  Eich dit s'être inspiré de Awk, C, HyperTalk, Self (pour
les prototypes), Scheme (pour les fonctions de première classe), et Perl (pour
les expressions rationnelles).  Contrairement à Java, JavaScript n'est pas un
langage à classes.  L'unité structurante majeure est la fonction:
...
Comme les tableaux, les objets peuvent être modifié à n'importe quel moment.  On
peut ajouter une association, la supprimer, ou modifier la valeur associée à un
nom.  Notons la syntaxe pour appeler une fonction associée dans un objet
(~obj.c()~), qui est identique à l'invocation d'une méthode en Java.  Mais la
sémantique est bien différente: ici la fonction n'est pas liée à l'objet par
construction.  On pourrait tout aussi bien l'invoquer de la sorte:
...
Néanmoins, cette structure associative peut être utilisé pour représenter des
...
Ici on a deux objets, ~A~ et ~B~.  ~A~ contient une seule fonction, ~a~.  ~B~
contient une seule fonction, ~b~, mais déclare ~A~ comme prototype.  En
conséquence, ~a~ n'est pas une fonction déclarée sur ~B~, mais ~B.a()~ appellera
la fonction ~a~ qui existe dans son prototype, ~A~.  Le mécanisme de prototype
fonctionne comme une liste chaînée...
...
prototypes, à l'exception du clonage:
...
** UNREAD Étude de cas: instrumentation ad-hoc de Narcissus
:PROPERTIES:
:CUSTOM_ID: narc
:END:
...
fonction retourne ~false~.  La fonction peut donc extraire toute l'information
de ~x~ sans jamais directement copier sa valeur.
...
condition.  Après le second ~if~, la fonction retourne la valeur ~true:false~.
Un observateur non autorisé n'a accès qu'à la valeur publique du résultat, et
n'est donc pas capable d'inférer la valeur privée de ~x~.  L'évaluation
multi-facettes permet de protéger l'information de ~x~, même contre les flots
indirects.
...
deux versions.  La figure suivante donne une vue d'ensemble de ces changements:
...
#+CAPTION: Narcissus pour l'évaluation multi-facettes.  Chaque ligne du diff
#+CAPTION: correspond à une ligne colorée suivant le type de changement dont
#+CAPTION: elle fait partie.  Les lignes grises ne sont pas affectées par
...
Résultat, il devient difficile de comprendre les effets de l'instrumentation à
l'œil nu, ou de s'assurer de sa justesse par rapport à une spécification.  Il
devient difficile également, sans connaissances avancées de Narcissus et de
l'évaluation multi-facettes, de savoir si une ligne de code de l'interpréteur
instrumenté concerne l'interprétation décrite par le standard ECMAScript, ou si
elle concerne l'évaluation multi-facettes.  Le code de l'interpréteur
instrumenté ne comporte aucune information qui permet de les distinguer.
...
** UNREAD Le but: factoriser les changements
...
Lorsque l'on parle de modifier l'interpréteur, comme dans la section précédente,
c'est que l'on modifie cette spécification.  Cette spécification modifiée
correspond à une nouvelle fonction:

: spec_2: source -interpréteur modifié-> processus

C'est un premier point de vue: l'interpréteur modifié est vu comme un nouvel
interpréteur, sans aucune relation avec le premier interpréteur.  En suivant ce
point de vue, il est naturel d'implémenter l'interpréteur modifié comme un
nouveau programme.  Le nouveau programme est basé sur le premier, donc on peut
réutiliser une bonne partie du code source du premier.  Mais une fois les
modifications effectuées, les deux programmes sont bel et bien indépendants.

Mais on peut aussi adopter un point de vue complémentaire et s'intéresser à la
/modification/ elle-même:

: interpréteur -modification-> interpréteur modifié

#+BEGIN_side-figure
#+CAPTION: Un rectangle peut être décrit par quatre points, ou par une
#+CAPTION: origine et un vecteur.  La seconde représentation est plus
#+CAPTION: compacte que la première, sans perte d'information.
...
Mais comment cette représentation différentielle se traduirait dans le code
source?  Rappelons nous que le processus de développement est le suivant:
...
la spécification: le fonctionnement attendu de l'interpréteur est modifié.  Le
cycle de développement est le suivant:
...
On a une spécification et un addendum, un ajout qui décrit une variante de
comportement de la spécification.  C'est le /delta/ mentionné plus haut.  Pareil
pour le code source: il n'y a plus deux codes, mais un code d'origine, et un
autre code qui vient modifier le premier.  Ce code peut engendrer deux
programmes différents: le programme d'origine, ou le programme qui contient le
delta.  On retrouve alors les deux processus attendus, mais les similarités de
la spécification et du code source sont factorisées.
...
dans le code de l'interpréteur.

Et cette factorisation passe plus facilement à l'échelle.  Si l'on veut
appliquer non plus une mais quatre analyses sur Narcissus, on n'a pas à
dupliquer le code de l'interpréteur quatre fois, mais juste à décrire les quatre
deltas requis par les analyses.
...
Au lieu d'avoir les quatre interpréteurs différents, on peut juste conserver les
modifications et l'interpréteur de base.  C'est une factorisation qui élimine la
redondance de I_1.
...
** UNREAD Quatre axes de factorisation pour l'instrumentation
En observant le diff de l'instrumentation de plus près, on peut distinguer
quatre catégories de changements: les imports/exports, l'ajout du paramètre
/program counter/, la séparation de l'évaluation multi-facettes, et les ajouts à
l'objet ~global~.
...
valeur courante du /program counter/, ~pc~.  Voici un extrait du diff qui
illustre ce changement:
...
contrôle est transféré à du code interprété exécutable: lors de l'entrée dans
une fonction, lors d'un appel à ~eval~, ou lors de l'exécution d'un programme
...
Les changements effectués dans l'exécution de l'arbre syntaxique abstrait pour
propager les étiquettes sur les valeurs à facettes.  Par exemple, additionner
deux valeurs à facettes devrait produire une nouvelle valeur à facettes.  Dans
l'implémentation, plutôt que de simplement additionner les deux opérandes,
l'interpréteur doit maintenant d'abord inspecter l'opérande gauche, et si c'est
une valeur à facettes, il faudra ajouter la valeur de l'opérande droite à chaque
facette.  Bien sûr, l'opérande droite peut également être une valeur à facettes,
et il faut alors séparer l'évaluation à nouveau.  L'interpréteur Narcissus ne
contient aucun code pour gérer l'addition de deux valeurs à facettes, donc
l'instrumentation doit ajouter la logique nécessaire.  Pour ce faire, chaque
évaluation d'une opération est enrobée dans un appel à la fonction
~evaluateEach~ qui teste si une valeur est à facettes, et appelle récursivement
la fonction d'évaluation sur chaque facette si c'est le cas.  25 appels à
~evaluateEach~ ont été ainsi ajoutés dans l'instrumentation.  Le code suivant
donne la forme générale de ces changements:
...
gauche d'une affectation, ou la condition d'un ~if~) puis on fait quelque chose
...
~globalBase~ avec les propriétés qui viennent remplacer celles de
l'environnement hôte.  Deuxièmement, il crée un objet ~global~ client à partir
de l'objet ~global~ de son environnement hôte, et met toutes les propriétés de
~globalBase~ dans cet objet ~global~ client.  Troisièmement, il réutilise les
objets ~Array~, ~String~ et ~Function~ de l'environnement hôte, et les ajoute à
l'objet global client.
...
catégories suggère des axes potentiels de factorisation.  Factoriser permettrait
de réduire la duplication de code, et surtout de regrouper les changements de
même nature en un seul endroit.  Par exemple, si l'interpréteur disposait d'une
fonction pour ajouter une valeur à l'objet global, tous les ajouts à cet objets
requis par l'instrumentation pourraient être effectués au même endroit:
...
On dispose maintenant de quatre axes de factorisation, mais une question
demeure: comment utiliser ces axes pour effectivement factoriser Narcissus?

** UNREAD Refactoriser Narcissus: une solution coûteuse
La factorisation évoque immédiatement une solution: la /refactorisation/.  En
programmation, refactoriser c'est "changer la structure interne d'un programme
pour le rendre plus simple et plus facile à modifier sans changer son résultat"
[[cite:Fow-99][Fow-99]].  Par changements successifs de code, le programme est restructuré pour
être plus clair et plus extensible, sans que son comportement observable n'en
soit affecté.
...
La refactorisation est justifiable, car comme on l'a vu, s'il y a une infinité
de programmes différents qui ont le même résultat, ils ne sont pas tous
identiques quand il s'agit de les modifier.  Heureusement, le programme est
modifiable.  La refactorisation a pour but de guider la restructuration du
programme par de petits changements successifs.  Ces changements, pris un à un,
sont simples à effectuer (presque mécaniques), et il est facile de se convaincre
(ou d'observer) qu'ils ne changent pas le comportement du programme.  À
l'inverse, si on restructure le programme entier ou de grandes parties d'un seul
tenant, on fait plutôt de la /réécriture/.
...
Alors, si l'on souhaite factoriser l'interpréteur en suivant les quatre axes de
la section précédente, la refactorisation est une solution légitime.  On pourra
utiliser différentes techniques (patron visiteur, /hooks/...), qui nous
conduiront à bien séparer l'interpréteur d'origine de l'évaluation
multi-facettes.

Mais refactoriser, par définition, c'est restructurer le programme.
Restructurer implique de changer le code source.  Donc refactoriser
l'interpréteur demandera un investissement non négligeable, quelles que soient
les techniques employées.  De plus, rien ne nous garantit que les axes de
factorisations qui sont indiqués par l'évaluation multi-facettes seront aussi
des axes de factorisation pour d'autres analyses.  Si l'on souhaite appliquer
d'autres analyses à Narcissus, on risque de dupliquer l'effort de
refactorisation.  La coût de la refactorisation deviendrait plus important que
le coût de simplement dupliquer les interpréteurs pour chaque analyse.

** UNREAD L'idée: le détournement de programme
Pour pallier aux coûts de la refactorisation, nous proposons une alternative: le
/détournement/.

Le détournement est basé sur un principe simple: plutôt que de modifier le code
source du programme, on va manipuler son contexte d'exécution.  Le contexte d'un
programme est aussi important que contexte d'une phrase en français: suivant le
contexte, le sens de la phrase peut énormément varier.

[super exemple]

J'irais même jusqu'à dire que sans aucun contexte, la phrase n'est qu'un
ensemble de traits vide de sens.  Il suffit d'imaginer ce que signifie la phrase
pour un non-francophone et non familier avec l'alphabet latin.  Ou encore, de
constater que le sens de cette phrase en sumérien est perdu à jamais:

[sumérien ?]

Comme les phrases, les programmes ont un contexte.  Comme les phrases, ce
contexte est généralement implicite, et il détermine entièrement le résultat du
programme.  Un code source n'a de sens que lorsque l'on donne le langage de
programmation dans lequel il est écrit.  Et encore, le nom du langage est
insuffisant: on doit se référer à sa sémantique, ou au modèle mental que l'on
s'en fait.  Différents compilateurs peuvent donner différentes sémantiques aux
programmes.  Et la machine qui exécute le programme peut encore en altérer le
comportement.  Le contexte du programme est donc défini par toutes les couches
d'interprétation qui séparent le code source du processus engendré par la
machine.
...
changer son code source, de la même façon qu'on peut changer le sens d'une
phrase en la plongeant dans un contexte différent, et sans en changer une
lettre.

On pourrait détourner de la sorte n'importe quel programme.  Mais le but ici,
c'est de détourner des programmes pour mieux les factoriser.  Le détournement
nous permet d'exprimer exactement la modification, le /delta/ d'un changement,
sans avoir à toucher le code source du programme.  Le détournement permet de
...
** Réifier le langage pour séparer les préoccupations [2/3]
...
** Tisser les facettes d'un artefact d'innombrables dimensions [3/3]
#+BEGIN_epig
...
** La modularité du programme n'est pas la modularité du code source [2/3]
...
* Construire un interpréteur par modules [0/4]
...
On a vu précédemment que résoudre le problème de l'expression dans un langage
...
rappelle le patron /Interpréteur/ [[cite:GHJ+94][GHJ+94]].  Chaque terme du langage sera
représenté par un objet qui comportera une fonction pour chaque opération sur le
langage.
...
Le second terme du langage représente l'addition entre deux termes.
...
** UNWRITTEN Modifier des opérations
...
simple affectation.  On a utilisé l'affectation pour ajouter ~show~, maintenant
...
Considérons une variante.  Plutôt que de retourner le double de ~n~,
...
** UNWRITTEN Passer de l'état aux opérations
