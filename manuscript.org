...
Comme dans Narcissus, le gros de la logique de l’interpréteur réside dans la
fonction ~execute~.  Les définitions qui précèdent réifient des objets de la
spécification: ~Address~, ~bottom~, ~FunctionObject~.  On réutilise
le nom ~ExecutionContext~ pour indiquer l’objet qui contient les environnements
de variables et qui est passé au fil des appels récursifs à ~execute~.
...
#+BEGIN_SRC js :results output stderr :noweb yes
...
#+BEGIN_SRC js :noweb yes :tangle lambda-standard.js
<<lambda-standard/ast-nodes>>
<<lambda-standard/interpreter>>
#+END_SRC

#+BEGIN_SRC js :noweb yes :tangle lambda-facets.js
<<lambda-facet/ast-nodes>>
<<lambda-facet/facet-definitions>>
<<lambda-facet/interpreter>>
#+END_SRC
...
git diff --no-index -- lambda-standard.js lambda-facets.js; exit 0
#+END_SRC

#+RESULTS:
#+BEGIN_SRC diff
diff --git a/lambda-standard.js b/lambda-facets.js
index 2d12239..6956c17 100644
--- a/lambda-standard.js
+++ b/lambda-facets.js
@@ -6,6 +6,51 @@ function app(fun, arg) { return {type: 'APP', fun, arg} }
 function ref(e) { return {type: 'REF', e} }
 function deref(e) { return {type: 'DEREF', e} }
 function assign(left, right) { return {type: 'ASSIGN', left, right} }
+function Facet(k, vh, vl) {
...
+}
 var interpreter = (function(){
   var bottom = {
     toString() { return 'BOT' },
@@ -23,10 +68,12 @@ var interpreter = (function(){
     if (parent) {
       this.scope = Object.create(parent.scope)
       this.store = parent.store
+      this.pc = parent.pc
...
@@ -46,6 +93,10 @@ var interpreter = (function(){
     this.scope = scope
   }
...
+
   FunctionObject.prototype._call = function(arg, context) {
     var x = new ExecutionContext(context)
     x.scope[this.node.argName] = arg
@@ -55,7 +106,7 @@ var interpreter = (function(){
   function execute(node, context) {
     var v, f, a, x, r
...
+    switch (node.type) {
     case 'BOT':
       v = bottom
       break
@@ -75,18 +126,19 @@ var interpreter = (function(){
     case 'APP':
       f = execute(node.fun, context)
       a = execute(node.arg, context)
-      v = f._call(a, context)
...
@@ -97,8 +149,15 @@ var interpreter = (function(){
     return execute(ast, new ExecutionContext())
   }
