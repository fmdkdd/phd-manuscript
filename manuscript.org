...
- using GOTO and COMEFROM to change a block of code at a distance without
  touching that code.
- without COMEFROM, can use two GOTOs to patch a routine, but need to insert a
  GOTO in the routine.
-> Interpreter in a language that has GOTO
- GOTO available only in a restricted form in JavaScript however
